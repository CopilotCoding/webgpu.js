...
try {
  ...
  // Two objects sharing one Material (one compiled pipeline), each with
  // its own MaterialInstance — separate tint + transform bind groups.
  const objects = [
    { x: -1.5, tint: new Float32Array([1.0, 0.3, 0.3, 1.0]) },
    { x: 1.5, tint: new Float32Array([0.3, 0.6, 1.0, 1.0]) },
  ].map(({ x, tint }) => {
    const instance = material.createInstance({ tint });
    return { x, instance };
  });
  ...
  function frame() {
    ...
    for (const object of objects) {
      object.instance.set('transform', translation(object.x, 0, 0));
    }
